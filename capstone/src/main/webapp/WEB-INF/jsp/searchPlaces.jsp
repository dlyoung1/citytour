	...
	<script>
		...
		function renderMap() {
			...
			if ($('#entertainment').is(':checked')) {
				selectedTypes.push('amusement_park', 'aquarium', 'casino',
						'zoo', 'movie_theater');
			}
			...
			if ($('#shopping').is(':checked')) {
				selectedTypes.push('clothing_store', 'department_store',
						'store', 'shopping_mall');
			}
					...
					.geocode(
							...
							function(results, status) {
								if (status === 'OK') {
									...
									selLocLat = results[0].geometry.location
											.lat();
									selLocLng = results[0].geometry.location
											.lng();
